...
export function FormTextArea({ name, label, required, icon: Icon, rules, className, ...props }: TextAreaProps) {
  const {
    register,
    formState: { errors },
  } = useFormContext();
  ...
  return (
    ...
    >
      ...
      <div className="relative group">
        {Icon && (
          <div className="absolute left-3.5 top-3 text-muted-foreground group-focus-within:text-blue-600 transition-colors pointer-events-none">
            <Icon size={16} />
          </div>
        )}
        <textarea
          ...
          id={name}
          className={cn(
            'flex min-h-[100px] w-full rounded-lg border border-border bg-white/50 dark:bg-black/20 py-2.5 text-sm transition-all resize-none',
